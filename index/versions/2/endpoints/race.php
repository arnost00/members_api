...
class Race implements Endpoint {
    ...
    public static function signin($race_id, $user_id) {
        ...
        if ($output === null) {
            ...
            Database::query(
                "INSERT INTO `" . Tables::$TBL_ZAVXUS . "` (`id_user`, `id_zavod`, `kat`, `pozn`, `pozn_in`, `transport`, `sedadel`, `ubytovani`, `termin`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
                $user_id,
                $race_id,
                $input["category"],
                $input["note"],
            ...
            // else, update the row
            Database::query(
                "UPDATE `" . Tables::$TBL_ZAVXUS . "` SET `kat` = ?, `pozn` = ?, `pozn_in` = ?, `transport` = ?, `sedadel` = ?, `ubytovani` = ?, `termin` = ? WHERE `id` = ?",
                $input["category"],
                $input["note"],
                $input["note_internal"],
                $input["transport"],
